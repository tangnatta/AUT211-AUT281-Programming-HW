...
public:
     ...
     * @param note_duration_arr (int) : The array that have {NOTE, DURATION, NOTE2, DURAION2, ...}
     * @param note_quantity (int) :  The number of note in array can caculate with sizeof(melody) / sizeof(melody[0]) / 2.
     */
    void play_array(int note_duration_arr[], int note_quantity)
    {
        // int note_quantity = sizeof(melody) / sizeof(melody[0]) / 2;
    ...
    /* Return the duration of note in millisecond. corespon to the tempo.
     * @param duration The duration of the note.
     */
    int get_note_duration(int duration)
    {
        return (duration > 0) ? (this->_whole_note / duration) : ((this->_whole_note / -(duration)) * 1.5);
    }
     ...
     */
    void play_note(unsigned int note, int duration)
    {
        // note durations: 4 = quarter note, 8 = eighth note, etc.:
